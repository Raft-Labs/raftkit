...
Read at once: the story and all its subtasks, its followers, and the Feature Template (once per conversation).
...
Above the diff: readiness gaps as numbered questions filled only from the PM's reply (the go is refused while one is blank), and when `Development` or `Testing` is ticked, one line: `This story is already being built; amending changes the agreed scope under the developer.` The go covers it.
...
Re-check readiness only when the description changed. Confirm:
