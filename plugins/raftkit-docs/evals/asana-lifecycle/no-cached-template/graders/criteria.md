...
- States that the story and bug templates are fetched live from Asana at run time, referenced by their GIDs (Feature Template `<Feature Template GID>`, Bugs Template `<Bugs Template GID>` — naming at least the mechanism; exact GIDs are a bonus, not required).
