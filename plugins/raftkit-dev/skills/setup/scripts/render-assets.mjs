#!/usr/bin/env node
// Fail-closed renderer for setup's pack assets. Operates on the SHIPPED
...
if (pm === "none" && scripts.length) fail("--pm none cannot carry --scripts — a non-Node posture has no manager to run them, and rendering this combination would ship a hook that blocks every push");
