...
| simplification-engine | code-simplifier | claude-plugins-official | agent: code-simplifier | required (declared) | raftkit-dev |
...
| ui-implementation | frontend-design | claude-plugins-official | skill: frontend-design | conditional: story has UI scope | raftkit-dev |
| ui-polish | impeccable | impeccable | skill: impeccable | optional | raftkit-dev |
...
| encrypted-env | envx | — | agent skill: envx | conditional: .envxrc / .env.*.gpg / Profile declaration | raftkit-dev |
| memory | claude-mem or remember | — | one-of: claude-mem, remember | optional (one only; show context cost) | raftkit-dev |
| skill-discovery | Skills CLI + find-skills | — | cli: npx skills find / list --json | required capability (CLI; suggestions only, never installs) | raftkit-dev |
