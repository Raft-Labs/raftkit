...
# PyYAML structural check on the generated YAML (if available — checks the
# module is actually importable, not just that python3 exists, so a
# python3-without-pyyaml environment skips quietly instead of a traceback).
