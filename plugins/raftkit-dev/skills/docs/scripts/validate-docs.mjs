...
function main() {
  ...
  if (opts.convention) {
    ...
    // The descriptor can name a convention discovery could not recognize
    // (d.convention was "none"), which unblocks the run — but discovery still
    // found no ownership table/footer to map from, so ownership stays empty.
    // Track that so a later "no changed path matches" is reported honestly
    // instead of as a verified no-impact (see the ownershipUnmappable check
    // below).
    if (d.convention === "none" && desc.convention) { d.convention = desc.convention; d.ownershipUnmappable = true; }
  }
  ...
  if (!changeSet) {
    ...
    if (impacted.size === 0 && d.ownershipUnmappable) {
      // The approved descriptor named a convention discovery could not derive
      // ownership from (no table/footer this validator recognizes) — never
      // report a verified no-impact off zero ownership evidence; that would
      // be an unconditional, machine-generated false green on every run.
      verdict = "Docs: not evaluated — approved descriptor's convention has no ownership mapping this validator can derive; impact must be confirmed by other means";
      evidence.push(`Inspected change set: ${changeSet.files.join(", ")} (${changeSet.source})`,
        `Documentation roots: ${d.docsRoot}`,
        "Ownership evidence: none found — descriptor-only convention, not verified");
