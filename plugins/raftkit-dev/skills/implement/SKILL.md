...
1. **Intake, in one turn**: the story and every `[AC]` subtask · the Feature Template if not already in this conversation · the Project Profile · `git fetch` of the target branch · the baseline build and typecheck. A red baseline stops here: fix it before touching feature code.
2. **Check readiness** inline against the fetched template (`raftkit-core:rules` → `references/readiness.md`). A gap the developer can answer is asked in the plan message and logged to the story at the stop. A gap that is commercial, client-facing, or that the developer will not answer ends the run with the gap list drafted as a story comment. That is the only early exit.
3. **Plan, in the open.** Scope is the `[AC]`s and nothing else. Phases are small enough to compile and test alone, each naming its files, its tier and its tests. Write it to `docs/specs/<branch>.md` and show it. It is a record, not a gate: the developer interrupts if it is wrong, and `--plan-only` stops here having written nothing but the record.
4. **Build.** Branch by the release-train convention, then run the phases: independent phases in parallel, dependent ones in order, each as a subagent given only its files and the story text it needs. Every phase goes red first, one failing test per acceptance criterion, then green. Small conventional commits. A subagent that cannot fix the same error three times stops and reports.
...
On a branch that already has the work: "raise the PR" runs step 6 onward, "check scope" runs `scope-guard` alone, "simplify this" runs the simplify pass alone. Each reuses the story already in this conversation rather than fetching it again. Merging is always human, and this skill never ticks `[AC]` or `Testing`.
