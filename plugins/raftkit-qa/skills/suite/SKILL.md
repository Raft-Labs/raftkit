...
The project's Profile (found by the rules' convention; none → stop: `No Project Profile found — run raftkit-pm:profile first, then re-run suite.`) and where the Sheet lives (QA names it once; one project per Sheet).
...
4. **Stop once.** Show the additions, the deltas, and every conflict side by side. Conflicts resolve in one reply: "accept all generated except …" or "keep QA's on all except …" is enough. Past the soft cap, propose a split by feature area in the same message.

```output
Suite draft for <project>: 42 new, 3 deltas, 2 conflicts (both versions below).
