...
appended to the project Sheet, these are its columns; in chat, the same fields
...
success and error copy (section 7 / section 6), its `[AC]` text, its Gherkin
...
- **WEESLD** — one step group per state the story specifies in section 6: Waiting,
  Empty, Error, Success, Limits, Default values. A state the story marks `N/A` is
