...
- **A dispatched subagent runs on the cheapest model that can do its job.** Where the runtime lets a skill name the model for work it dispatches, the skill names it from Protocol 1's tier table — read the tiers live from [governance-protocols](../governance-protocols/references/protocols.md), never from a copy here. Pick the lowest tier that can do the work. When the right tier is genuinely unclear, **say so and ask** — name the phase and the choice you are between, and let the human decide. Never resolve an unclear tier by inheriting the session's model: a session sitting on Opus turns every unclear call into the most expensive one, which is the spend this rule exists to stop.

  **Scope, and how this squares with Protocol 1.** This governs dispatched work only. Nothing here changes the model of the session a skill is running in — that stays the developer's to set, and Protocol 1's efficiency warning is how a skill raises it. Protocol 1 also says models cannot be switched programmatically mid-session; that remains the governing text for the session model, and naming a dispatched subagent's model is a different act. The full reconciliation, and who owns amending the protocol, is recorded under [governance-protocols](../governance-protocols/SKILL.md)' pending-amendment note — read it before relying on either document alone.
