...
- **Work is matched to the cheapest model that can do it, and the human does the switching.** Protocol 1 asks that model capacity be matched to task difficulty; this is how a skill acts on that. Read the tiers live from [governance-protocols](../governance-protocols/references/protocols.md), never from a copy here, and work out the lowest tier that can do the work. Then **say it and let the human act**: when the session is running a more expensive model than the work needs, name the cheaper tier and the command to switch, per Protocol 1's efficiency warning. When the right tier is genuinely unclear, say that instead — name the choice you are between and let the human decide. Never quietly run expensive work you could have flagged as cheap.
