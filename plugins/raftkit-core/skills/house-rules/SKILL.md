...
- **A dispatched subagent runs on the cheapest model that can do its job.** When a skill hands work to a subagent, it names the model for that subagent from the tier table in [governance-protocols](../governance-protocols/SKILL.md) (Protocol 1: Haiku for mechanical work, Sonnet as the default workhorse, Opus or Fable only for architecture and cross-file reasoning). Pick the lowest tier that can do the work; when the tier is genuinely unclear, say nothing and let the subagent inherit the session's model rather than guessing upward. This applies only to work a skill dispatches — the developer's own session model is theirs to set, and Protocol 1's efficiency warning is how a skill raises it.
