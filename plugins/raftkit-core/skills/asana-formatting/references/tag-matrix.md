...
- **`<br/>` is avoided everywhere** — it is usually stripped or mangled. Get
  vertical spacing from block elements (`<hr/>` where allowed, `<ul>`, another
  `<strong>` line, a heading).
