...
RaftKit is RaftLabs' private Codex plugin **marketplace**: one repo shipping four plugins that package the RaftLabs way of delivering software. The plugins **orchestrate** proven third-party plugins (superpowers, code-simplifier, ponytail, pr-review-toolkit, security-guidance, CodeRabbit, find-skills, asana, Codex-md-management, remember, frontend-design, playwright/chrome-devtools, expo, neon) — **they rebuild nothing**. Asana is the workflow spine: work enters as a templated user story and exits as a squash-merged PR with QA-verified acceptance criteria.
...
2. **PRD** — [Codex-plugin-marketplace-prd.md](https://drive.google.com/file/d/1nJrBdvUIizJme9ysDAJPNnF0waKrra4R/view) (also in Google Drive → RaftLabs - General → Raftlabs Framework). Architecture, skill specs, guardrails, metrics.
...
```
.Codex-plugin/marketplace.json     # single source of what is installable
plugins/
  ...
  raftkit-dev/    # implement, scope-guard, simplify, pr, fix-bug, ui-creation,
                  # setup-project, fix-production-error, recipes   (Codex)
  raftkit-qa/     # test-suite, test-run-sheet, file-bug, retest        (Cowork)
```

Each plugin: `.Codex-plugin/plugin.json` + `skills/<skill-name>/SKILL.md`. Verify manifest/marketplace schema against the current Codex plugin docs before scaffolding — do not trust memory.
