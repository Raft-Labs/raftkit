...
- **Human gates everywhere:** skills draft, humans approve — story approval, plan approval, PR merge, bug close. No skill ever auto-sends, auto-merges, or auto-files. Exactly two exceptions, both enumerated in `raftkit-core/house-rules` and `write-protocol`: raftkit-core's telemetry hooks auto-file blocker issues on this repo's own tracker only — never a client repo — and the opt-in `pr-auto-review` CI workflow auto-commits Critical-finding fixes to the PR branch it runs on, never merging.
