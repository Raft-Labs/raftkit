#!/usr/bin/env bash
# Contract suite for raftkit-pm:feature-brainstorm and the shared interview
...
FB=plugins/raftkit-pm/skills/feature-brainstorm
...
joined "$FB/SKILL.md" | grep -qi 'One line is enough' \
...
grep -qi 'never from memory and never' "$FB/SKILL.md" 2>/dev/null \
  ...
  && joined "$MAP" | grep -qi 'maps \*\*topics\*\* to question sets'
...
grep -q '| `feature-brainstorm` |' plugins/raftkit-pm/commands/help.md 2>/dev/null \
