...
# Structural YAML checks below need PyYAML, not just python3 — check both
# once so a python3-present-but-no-pyyaml environment skips that half of a
# check gracefully instead of a raw traceback reading as a real failure.
PYYAML_OK=0
...
for pm in npm pnpm yarn bun; do
  ...
  [[ $RRC -eq 0 ]] && ! grep -qE '__[A-Z_]+__' "$ODIR/pre-push" "$ODIR/quality-guardrail.yml" \
    && sh -n "$ODIR/pre-push" \
    && { [[ "$PYYAML_OK" -eq 0 ]] || python3 -c "import yaml,sys; yaml.safe_load(open('$ODIR/quality-guardrail.yml'))"; } \
